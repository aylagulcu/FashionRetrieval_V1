...
class FashionProductsDataset(Dataset):
    ...
    def __init__(self,img_dir,styles_dir, transform=None, ):
        ...
        assert(styles_dir is not None)
        assert(img_dir is not None)


        df = pd.read_csv(self.styles_dir,sep=",", header=0,error_bad_lines=False, warn_bad_lines=False)
        
        self.ids = df["id"].to_numpy().tolist()
        self.genders = df["gender"].to_numpy().tolist() # class: string name
        self.master_categories = df["masterCategory"].to_numpy().tolist()
        self.sub_categories = df["subCategory"].to_numpy().tolist()
        self.articleTypes = df["articleType"].to_numpy().tolist()
        self.baseColors=df["baseColour"].to_numpy().tolist()
        self.seasons = df["season"].to_numpy().tolist()        
        self.years = df["year"].to_numpy().tolist()
        self.usages = df["usage"].to_numpy().tolist()
        self.productDisplayNames = df["productDisplayName"].to_numpy().tolist()
        
        self.classes = [i+"/"+j for i,j in zip(self.master_categories,self.sub_categories)  ]
     
        self.classes_set = list(OrderedDict.fromkeys(self.classes)) # given label integer; get the class name
         
        del df
                              
        self.idx_to_class = {i:j for i, j in enumerate(self.classes)} # given idx of the sample, get class name
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(self.img_dir + os.sep + str(img_path) + ".jpg")
        fixed_height = 300
...
class DeepFashionCoarseDataset(Dataset):
    '''
    ...
    def __init__(self,img_dir,train_path=None,test_path=None,validation_path=None,mode="train", transform=None, ):
        ...
        #self.ids = df["id"].to_numpy().tolist()
        self.main_classes = df["main_category"].to_numpy().tolist() # class: string name
        ...
        self.classes = [i+"/"+j for i,j in zip(self.main_classes,self.sub_classes)  ] # in the given order of the input
...
class DeepFashionDataset(Dataset):
    def __init__(self,img_dir,train_path=None,test_path=None,validation_path=None,mode="train", transform=None, ):
        ...
        self.landmarks = df["landmarks"].to_numpy().tolist()
        self.attributes = df["attributes"].to_numpy().tolist()
        self.classes = [i+"/"+j+"/"+k for i,j,k in zip(self.main_classes,self.sub_classes,self.ids)  ]
...
class DeepFashionBBoxDataset(Dataset):
    def __init__(self,img_dir,train_path=None,test_path=None,validation_path=None,mode="train", transform=None, ):
